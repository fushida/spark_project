...
object KafkaWordCountStateful {
  ...
  /**
    ...
    * 第三个参数：初始值或累加的中间结果（保存到hdfs中防止丢失）
    */
	
	//使用迭代器
  val updateFunc = (iter: Iterator[(String, Seq[Int], Option[Int])]) => {
    //iter.map(t => (t._1, t._2.sum + t._3.getOrElse(0)))//当前批次结果相加 +历史结果
  ...
  def main(args: Array[String]): Unit = {
    ...
    val conf = new SparkConf().setAppName("KafkaWordCountStateful").setMaster("local[*]")
    ...
    val groupId = "g100"
    val topic = Map[String, Int]("demo" -> 1)
