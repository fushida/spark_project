...
/**
  * 将相同学科的数据suffer到同一个分区中
  *
  * 自定义分区器：决定下游数据到上游中
  * partitionBy按照指定的分区规则进行分区
  */
object GroupSubject3 {

  def main(args: Array[String]): Unit = {

    val topN = args(1).toInt

    val conf = new SparkConf().setAppName("GroupSubject3").setMaster("local[*]")
    ...
    //聚合，将学科和老师联合当做key
    val reduced: RDD[((String, String), Int)] = sbjectTeacherAndOne.reduceByKey(_+_)  //这里会shuffer一次
    ...
    val partitioned: RDD[((String, String), Int)] = reduced.partitionBy(sbPatitioner)//这里也会shuffer一次
...
class SubjectParitioner(sbs: Array[String]) extends Partitioner {
  ...
  //相当于主构造器（new的时候回执行一次）
  //用于存放规则的一个map
  val rules = new mutable.HashMap[String, Int]()//可变的map集合
  var i = 0
  for(sb <- sbs) {
    //rules(sb) = i
    rules.put(sb, i)//拿出学科 编号加一，放进hashmap中
    i += 1
  }
  ...
  override def getPartition(key: Any): Int = {
    //获取学科名称
    val subject = key.asInstanceOf[(String, String)]._1//转换类型为元组，拿到学科名称._1
    //根据规则计算分区编号  对于学科，
    rules(subject)
  }
}
