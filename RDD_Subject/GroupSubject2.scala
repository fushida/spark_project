...
/**
  ...
  * 取出TopN
  */
object GroupSubject2 {

  def main(args: Array[String]): Unit = {

    val topN = args(1).toInt

    val subjects = Array("python", "java", "php","scala")

    val conf = new SparkConf().setAppName("GroupSubject2").setMaster("local[*]")
    ...
    //如果前面的cache数据用完后，释放数据  .unpersist()
    //对于放不下的数据如何处理：

    //scala的集合排序是在内存中进行的，但是内存有可能不够用
    //可以调用spark RDD的sortby方法，内存+磁盘进行排序
    ///----------------------------------------------------------------
    for (sb <- subjects) {
