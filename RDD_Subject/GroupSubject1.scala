...
/**
  ...
  * grouped.mapValues()
  */
object GroupSubject1 {

    def main(args: Array[String]): Unit = {

    //val topN = args(1).toInt

    val conf = new SparkConf().setAppName("FavSubject").setMaster("local[4]")
    val sc = new SparkContext(conf)

    //指定以后从哪里读取数据
    val lines: RDD[String] = sc.textFile(args(0))
    //整理数据
    val sbjectTeacherAndOne: RDD[((String, String), Int)] = lines.map(line => {//RDD写好类型
    ...
    //经过分组后，一个分区内可能有多个学科的数据，一个学科就是一个迭代器
    //将每一个组拿出来进行操作
    //为什么可以调用sacla的sortby方法呢？因为一个学科的数据已经在一台机器上的一个scala集合里面了   rdd只能全局排*******
    val sorted = grouped.mapValues(_.toList.sortBy(_._2).reverse.take(1))
