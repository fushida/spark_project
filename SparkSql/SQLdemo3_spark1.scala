...
/**
  * 
  */
object SQLdemo3_spark1 {

  def main(args: Array[String]): Unit = {

    //提交的这个程序可以连接到Spark集群中
    val conf = new SparkConf().setAppName("SQLdemo3_spark1").setMaster("local[2]")
    ...
    val lines = sc.textFile("hdfs://hadoop/person")
    //将数据进行整理
    ...
    //不使用SQL的方式，就不用注册临时表了
    val df1: DataFrame = bdf.select("name", "age", "fv")//相对于sql，执行查找更加简单

    import sqlContext.implicits._

    val df2: DataFrame = df1.orderBy($"fv" desc, $"age" asc)//排序         语言编程比较不一样
