...
/**
  * spark1.0
  */
object SQLdemo2_spark1 {

  def main(args: Array[String]): Unit = {

    //提交的这个程序可以连接到Spark集群中
    val conf = new SparkConf().setAppName("SQLdemo2_spark1").setMaster("local[2]")
    ...
    val rowRDD: RDD[Row] = lines.map(line => {
      ...
      Row(id, name, age, fv)//行   区别于demo1  转换成row形式
    })

    //结果类型，其实就是表头，用于描述DataFrame
    val sch: StructType = StructType(List(//集合
      StructField("id", LongType, true),//和前面的例子一样
