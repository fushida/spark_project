...
/**
  ...
  * 取到HDFS中的ip规则                         规则存储到hdfs中  然后取出 通过  Task 再进行广播
  */
object IpLoactionSQL2 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    spark.udf.register("ip2Province", (ipNum: Long) => {
      ...
      //根据IP地址对应的十进制查找省份名称
      val index = MyUtils.binarySearch(ipRulesInExecutor, ipNum)//调用查找的二分法规则
      var province = "未知"
      if(index != -1) {
    ...
    //若此时再写sql,反而变的复杂了    
    //spark.sql("select ip2Province(ip_num) province,count(*) counts from v_log group by province order by counts desc ")//自定义函数ip2Province
    //需要先定义UDF 再注册
