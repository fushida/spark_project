    ...
    def main(args: Array[String]): Unit = {
        ...
        sc.textFile(logInputPath)
          ...
          .map(arr => {
              ...
              val req = RDDUtils.caculateReq(log.requestmode, log.processnode)
              val rtb = RDDUtils.caculateRtb(log.iseffective, log.isbilling,log.isbid, log.adorderid, log.iswin, log.winprice, log.adpayment)
              val showClick = RDDUtils.caculateShowClick(log.requestmode, log.iseffective)
        ...
        parquetData.map(row =>{
            ...
            val reqList = RDDUtils.caculateReq(reqMode, prcNode)
            val rtbList = RDDUtils.caculateRtb(effTive,bill,bid,orderId,win,winPrice,adPayMent)
            val showClickList = RDDUtils.caculateShowClick(reqMode,effTive)
